...
public class Course
{
    ...
    public void CourseModification(string operation, Course courseBeingOperated)
    {
        ...
        {
            "insert" => InsertCourse(courseBeingOperated, db),
            "update" => UpdateCourse(courseBeingOperated, db),
            "delete" => DeleteCourse(courseBeingOperated, db),
    ...
    private Task<Course> GetCourseByIdAsync(string courseId, SQLiteAsyncConnection db)
    {
        return db.Table<Course>().Where(c => c.CourseId == courseId).FirstOrDefaultAsync();
    }
}